---
layout: post
title: Crosscompiling ROS for Raspbian (ARMv6)
date: 2019-04-13
---

This guide covers how to crosscompile ROS. This will cover some basic concepts of crosscompiling and will give some hints along the way.
...
It really makes sense to crosscompile in an environment which is similar to your target. Raspbian is basically just a Debian which supports ARMv6 and therefore the Raspberry Pi 1 and Raspberry Pi Zero. There can be some confusion with the term armhf (ARM hard-float), because Debian [means the ARMv7 architecture](https://wiki.debian.org/ArmHardFloatPort).
...
I prepared a folder structure for the crosscompiling. You can check it out using git:
```bash
...
Install some general dependencies and the dependencies we need for ROS:
```bash
...
I prepared a configuration, which is based on the o[riginal raspberry config](https://github.com/raspberrypi/tools/blob/1169511ac73c7bb89f79df2e2465b4b80fbafd2f/configs/arm-rpi-4.9.3-linux-gnueabihf.config), for crosstool-ng which builds a GCC 6.3. Compiling ROS with an older version fails.
...
Now you can mount in on `/mnt` and copy the needed files to `build_ros/crosscompile/sysroot`

```bash
cd /build_ros
mount -o ro,loop,offset=50331648 -t auto 2018-11-13-raspbian-stretch-lite.img /mnt
cd crosscompile
./copy-img-host.sh
umount /mnt
```
...
If we'd try to crosscompile ROS using that directory now it would fail because symbolic links are broken in the `sysroot`. To solve this you can do:
```bash
...
Now change back to the docker image and compile `gtest` and `console_bridge`:
...
mkdir tinyxml/build
cd tinyxml/build
cmake -D CMAKE_TOOLCHAIN_FILE=/build_ros/crosscompile/toolchain.cmake /build_ros/crosscompile/tinyxml/
make -j10
make install DESTDIR=/build_ros/crosscompile/sysroot
cd ../..
```
...
We can now prepare the docker container for the compilation. We need to install the same dependencies in the chroot (`sysroot`) as on docker, because the build process maybe needs x86 binaries which need to be available in docker.
...
The next command will install dependencies of ROS as root without using `sudo` (`--as-root apt:false`). It will also continue if there are errors (`-r`):
```bash
rosdep install --from-paths src --ignore-src --rosdistro melodic --os=debian:stretch --as-root apt:false -y -r
```

In order to install the same dependencies in the chroot we need to copy the catkin workspace to the `sysroot`:
```bash
rsync -rap . /build_ros/crosscompile/sysroot/catkin
```
...
It is fine if several ROS dependencies are not found. We need to compile these we well because they are not packaged.
...
```bash
cd /build_ros/crosscompile/catkin
./src/catkin/bin/catkin_make_isolated --install --install-space /build_ros/crosscompile/sysroot/opt/ros/melodic -DCMAKE_BUILD_TYPE=Release -DCMAKE_TOOLCHAIN_FILE=/build_ros/crosscompile/toolchain.cmake
```

If this fails it is probably because of some missing ROS dependency. In this case use the generator to add the dependency to the `catkin/src` directory:
```bash
rosinstall_generator $DEPENDENCY --rosdistro melodic --deps --tar > missing-dependencies.rosinstall
wstool merge -t src missing-dependencies.rosinstall
wstool update -t src -j8
```
If there are new dependencies you also need to run `rosdep install` again on docker and in the `sysroot` chroot. In my case this was needed when compiling `mavros`. 
...
For convenience I grabbed the dependencies of `mavros` and `raspicam` which need to be installed in docker and in the chroot:
...
Installation of geographiclib datasets:
```bash
...
You also need to compile [opencv3](https://github.com/opencv/opencv/archive/3.4.5.zip). Here is [how to do it](https://docs.opencv.org/3.3.0/d7/d9f/tutorial_linux_install.html) on Linux. This works similarly to the compilation of `console_bridge`. The `apt install` above already installs all the dependencies you  need.


# Prepare a Raspbian image for production

You have compiled now everything in the `sysroot` directory. You should resize the original Raspbian image now and install the dependencies there again.

```bash
dd if=/dev/zero bs=1M count=1024 >> raspbian-stretch-lite-ros.img
```

Now delete partition 2 and recreate it using fdisk. The starting block of the new partition should be the same as in the preivous partition.
```bash
...
```
make install DESTDIR=/build_ros/crosscompile/sysroot-release
```
<sup>Note: For opencv you need to run `cmake -DCMAKE_INSTALL_PREFIX=/build_ros/crosscompile/sysroot-release` again to install to an other directory!</sup>
...
My original crosscompilation notes notes can be found [here](https://gitlab.com/searchwing/development/searchwing-pi-ros/blob/master/original-install-notes.md).
