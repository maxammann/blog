...
In late 2020, a group of students from TUM, LMU and University of Augsburg and the Tür an Tür Digitalfabrik started the design and development of the Ehrenamtskarte Bayern app. From the beginning the development was open-source. After the offical introduction of the app at "Bayerisches Staatsministerium für Familie, Arbeit und Soziales", the concept was made generic such that it can be applied to other kinds of digital entitlement cards - the [entitlementcard](https://github.com/digitalfabrik/entitlementcard) project was born.

We knew that NRW was in the design process of developing a reference implementation of the OZG, because the OZG requires some transparency in the development process and therefore the current state of implementations are published in on the [Informationsplattform](https://informationsplattform.ozg-umsetzung.de/iNG/app/detail?id=103532&nav=RegKO_RO&tb=projectdetails&pager). We tried to reach out several times to the relevant people and companies in NRW to collaborate on the effort.
...
In our disclosure we found that there is no secure way to verify volunteers (EAK-1), which means that any person can create their own voluntter card and receive discounds in NRW.
The Staatskanzlei des Landes Nordrhein-Westfalen argued that the discounds available in the app do not incentivice fraud enough.
However, we do not agree with this as the lack of security in the NRW app incentivies discount provider, like a car rental company, to provide small or no discounts at all.

We believe that this is a failure of the OZG and digitization of governmental processes.
The Ehrenamtskarte Bayern and its generic whitelabel solution offers a secure and private open source solution - free of charge. The bavarian solution is licensed under the MIT license.
We still hope that eventually either the NRW Ehrenamtskarten-App implements a securite verification or considers forking the open source entitlementcard platform.
...
{{< resourceFigureVideo "nrw-2024-04-13_15.06.22.mp4" >}}Illustration of EAK-1{{< /resourceFigureVideo >}}

## Findings

We found the following security, privacy and legal issues in the [NRW Ehrenamtskarten-App](https://www.engagiert-in-nrw.de/app-zur-ehrenamtskarte-nrw). EAK-1 through EAK-6 affect the app verison from 2024/04/18. EAK-7 affects the version of the app from 2024/08/06. In each case the iOS and Android app are affected.
...
The app visually displays the first and last name of the cardholder, a validity date and a card number on the volunteer's device.
card number. The same information is also encoded in a QR code.

In order to scan the QR codes, acceptance partners are given instructions on an [FAQ page](https://www.engagiert-in-nrw.de/faq-zur-ehrenamtskarten-app-nrw#faq_question_4037) published by the Staatskanzelei NRW (STK NRW) advises acceptance partners to
advised to use the camera app integrated in the system or another app for general scanning of QR codes.
In the best case scenario, the acceptance partner can read the above data in plain text if the scanner app used supports this.
scanner app supports this. Here is a screenshot of the website:

{{< resourceFigure "Screenshot 2024-04-16 08-45-42 Ehrenamtskarte FAQ.png"  >}}Screenshot of the NRW website{{< /resourceFigure >}}

However, this mechanism does not provide verification of the data displayed in the QR code. Furthermore, the app
the QR code within the app and to verify the encoded data via a hash comparison with a database in the backend.
with a database in the backend. Another option would be to check a signature contained in the QR code
...
significant security features. However, with the digital version, it is possible with considerably less effort
possible to issue counterfeit cards on a large scale.
...
replace the volunteer's name with their own name. A QR code containing the following text is also created:
...
verification of the QR code. If the QR code is not scanned with the app feature (e.g. camera app),
then no data about the volunteer should be recognizable as plain text in order to avoid possible misuse.
...
A final alternative would be to use a secure open source solution like the [entitlementcard](https://github[.]com/digitalfabrik/entitlementcard) project that has correctly identified these attacks in its threat model.
correctly identified in its threat model.
...
The STK NRW confirmed this finding, but did decide not to fix it yet due to the low severity.
...
This data transfer is required to display the map data from OpenStreetMap. In the privacy policy
does not mention this fact. As a result, users are not sufficiently informed about how their data is used and shared.
...
The STK NRW confirmed this finding and does not longer transmit data to jsDelivr.

## Disclosure timeline

- 2024-04-18 - EAK-1 through EAK-6 were reported to the STK NRW. We set an the embargo date to 2024-07-31.
