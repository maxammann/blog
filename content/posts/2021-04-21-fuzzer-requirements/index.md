...
<!--  TODO
 Re. trace t1: minor comment but I wonder whether it wouldn't be simpler/cleaner to actually send structured TLS messages f(v_1,,,v_n) instead of encode(f(v_1,..,v_n)) in your example. My question has two levels:

* theoretically, this should be possible and has the advantage that it "hides" the encode and decode that are irrelevant as we are not interested in fuzzing them. Another advantage is that we do not have to restrict the attacker to send sane messages only -> he just has to send structured TLS message paylaod.
* could this be done in the implementation as well? I mean you create a message::Message with rusttls given a "recipe" generated by your fuzzer and you instantiate it as a TLS message as defined in the OpenSSL lib. Maybe this does not worth the effort though ... as using encode/decode allows to simply pass bitstrings from rustls to OpenSSL and vice versa.
-->
...
<!-- TODO
I don't see how the trace t2 describes the message that c will receive. I realize that you define something different from symbolic traces here. Is this intended?
-->
...
<!-- TODO
Again, in a symbolic trace, the computations the attacker need to do are entirely (symbolically) specified.

Which means, there is only one way to concretize the computation (assuming deterministic randomness). 

(See the messageHandle from my little grammar on the annotated PDF we discussed last time.)

(See also line 20 of your symbolic attack trace of https://maxammann.org/posts/2021/04/symbolic-model-guided/#big-picture: pkX refers to the received message and the color indicates which agent is receiving this adversary-constructed message.)
-->
